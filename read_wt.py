...
DENS_CHANGE = 1 # hours for mixture separation or vice versa
...
def read_WT_hist_file(file_name):
    ...
    if max(qoil)==0:
        ...
        dt_deltas = [x.total_seconds()/60/60 if x.total_seconds()/60/60 < DENS_CHANGE else 0 for x in deltas] # print hours from regime change within one hour
        #print(dt_deltas)
        ...
        for mix_den_i, dates_i, bhp_i,bhp_mix_i, wcut_i, qoil_i, qwat_i, thp_i, hours_i, dt_deltas_i in zip(mix_den, dates, bhp, bhp_mix, wcut, qoil, qwat, thp, hours, dt_deltas):
            #print(f"{well} {dates_i:%d.%m.%Y %H:%M:%S} {gauge_tvdss:.2f} {wcut_i:.3f} {bhp_i:.2f} {bhp_mix_i:.2f} {mix_den_i:.3f} {dt_deltas_i:.5f}") # debug format
